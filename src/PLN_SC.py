...
class IDFApp:
    def __init__(self, root):
        ...
        self.lbx_colunas.grid(row=1, column=0, columnspan=2, pady=10, padx=50, sticky="nsew")
        
        self.lbl_amostra = customtkinter.CTkLabel(root, text="Insira a Amostra:", font=estilo_titulo)
        self.lbl_amostra.grid(row=2, column=0, columnspan=2, pady=10, padx=20, sticky='w')
        
        self.txt_amostra = tk.Text(root, height=5, width=40, font=estilo_normal, bd=2, relief="solid")
        self.txt_amostra.grid(row=3, column=0, columnspan=2, pady=10, padx=20, sticky="nsew")
        ...
        self.btn_calcIDF.grid(row=4, column=0, columnspan=2, pady=20, padx=150, sticky="nsew")
        
        self.lbl_resultado = customtkinter.CTkLabel(root, text="Resultado:", font=estilo_titulo)
        self.lbl_resultado.grid(row=5, column=0, columnspan=2, padx=20, sticky='w')

        self.txt_resultado = tk.Text(root, height=8, width=60, font=estilo_normal, bd=2, relief='solid', state="disabled")
        self.txt_resultado.grid(row=6, column=0, columnspan=2, pady=10, padx=20, sticky="nsew")
        ...
        self.txt_elementos_analisados.grid(row=7, column=2, columnspan=2, pady=10, padx=20, sticky='nsew')
        ...
        try:
            ...
            for i in ordens:
                ...
                if porcentagem_similaridade > 0 and ordens.size != 0:
                    ...
                    self.txt_resultado.insert(tk.END, f" Similiaridade( º ): {grau_similiaridade:.4f} º\n\n")
                else:
                    self.txt_resultado.delete(1.0, tk.END) 
                    self.txt_resultado.insert(tk.END, f"Top 5 Elementos Mais Semelhantes à amostra(%): ({nome})\n\n")
